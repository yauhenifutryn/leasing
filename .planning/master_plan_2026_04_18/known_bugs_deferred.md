...
| 32 | Item-aware parameter collection (non-car subjects ask car questions) | [03_5_calculator_funnel.md](03_5_calculator_funnel.md) — extension | Live transcript 2026-05-08 chat: user asked "недвижимость … склад и я бизнес"; bot followed with "новый или подержанный" (a car-specific question). The parameter set the bot collects today is hardcoded car-style (subject/condition_new/age_years/cost/term/prepaid/schedule). Real estate, equipment, and special-vehicle leasing have different per-subject parameter sets — the calculator API exposes these via `/subjects/` and `/ranges/`, which §3.5 already plans to consume. The fix is to extend §3.5's `CalcLimitsAdapter` to drive the question-collection order from the live API per chosen subject, not from a hardcoded car script. Until §3.5 ships, real-estate / equipment leasing remains a marked edge case — the bot will collect car-style fields and `/calculate/` will 404 on subject mismatch. Client review still owed for the wording of the subject-specific intake prompts. |
