...
- [ ] **"Тот, что дешевле" → schedule choice** — live call eb3d0a3d (2026-04-27): bot explained linear is cheaper, user said "тот, что дешевле", LLM narrated linear + readback inline but didn't stage deterministic type_schedule="1". User's "Верно" then hit the unfilled-field clarify and looped. Fix: extend SessionAgent classifier prompt to map "тот/тот же/такой/что дешевле/лучше/выгоднее/проще" + open type_schedule clarify → emit change_field="type_schedule", change_value (linear=1 for "дешевле/выгоднее", annuity=0 for "удобнее/проще"). Pure prompt-side, semantic, ~10 lines. Same shape as the 2026-04-27 mid-collection RAG-drift priority block (commit 2a1d192).
- [ ] **Age vs term collision in same turn** — call eb3d0a3d (2026-04-27): worked correctly in this case ("Два года и давай срок на три года, а аванс 36%" → age=2 + term=36) thanks to existing keyword-cue disambiguation. Open edge case: user says ambiguous "Два года и три года" with no "на/срок" anchor. Fix: when classifier emits both age_years and term_months from one utterance AND no anchor word, defer to order-of-questions (bot asks age before term, so first number = age). When still ambiguous, emit deterministic clarify ("Два года — это возраст машины, а три года — это срок лизинга, верно?"). Dispatcher-side, in `turn_dispatcher.py` after the year-form disambiguation block. ~20 lines.
