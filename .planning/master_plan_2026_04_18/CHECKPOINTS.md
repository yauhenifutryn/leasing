...
- [x] **"Тот, что дешевле" → schedule choice (Polish A)** — DONE 2026-04-27. Two-piece semantic fix: prompt-side `bf113e6` (РАЗРЕШЕНИЕ ССЫЛОК block) + dispatcher-side `c1149c5` (intent=TOOL change pair bypasses verbatim grounding). +2 unit tests. Verified end-to-end via simulator.
- [ ] **Age vs term collision in same turn (Polish B)** — call eb3d0a3d (2026-04-27): worked correctly in this case ("Два года и давай срок на три года, а аванс 36%" → age=2 + term=36) thanks to existing keyword-cue disambiguation. Open edge case: user says ambiguous "Два года и три года" with no "на/срок" anchor. Brutal honesty: no smart semantic fix exists for the no-anchor case. Recommended approach: dispatcher-side ambiguity detector → deterministic clarify ("Два года — это возраст машины, а три года — это срок лизинга, верно?"). ~30 lines, additive, future polish wave.
- [x] **Semantic type_schedule + inverted year forms (Polish C+E)** — DONE 2026-04-27 (commit pending push). Live call 56c0e2f9: user said "лет на 5" + "чтобы платежи были равными" + "нулевым авансом" in one turn. Bot captured only prepaid=0; missed term and type_schedule. Fix: extended SessionAgent classifier prompt with inverted year-form examples ("лет на 5" → 60) and semantic schedule synonyms ("равные/одинаковые/фиксированные платежи" → annuitet "0"; "уменьшающиеся/убывающие/тело долга гасится быстрее" → linear "1"). Pure prompt-side, ~15 lines.
- [x] **Calc-fail non-silence guard (Polish D-lite)** — DONE 2026-04-27 (commit pending push). Live call 56c0e2f9: calculator returned API error; bot spoke verbatim "По заданным параметрам условия лизинга не найдены." with no follow-up question — user perceived as silence. Fix: `_format_calc_error` and `_CALC_CIRCUIT_OOR` always end with "Хотите изменить срок, аванс или стоимость?" so user always has obvious next move. NOTE: structured per-constraint guidance (which param actually broke the request) deferred to Section 3.5.
- [ ] **Calc-fail structured-error parsing (Polish D-full)** — live call 56c0e2f9: prepaid=0 + term=60 combo failed. Section 3.5 (Calculator Funnel API Integration) is the canonical place for this — that section already plans tier-aware caps and structured error surfaces. Don't patch ad-hoc; fold into Section 3.5 work.
- [x] **KB office-address synthesis + RAG content bleed (Polish F)** — DONE 2026-04-27 (commit pending push). Live call d172e0f0: bot answered "адреса офисов" with only 2/6 cities AND injected unrelated "коммерческой недвижимостью / жилой" line. KB audit: all 6 city addresses present and clean (Минск Победителей-57, Могилёв Комсомольская-10а, Брест Машерова-6а, Гомель Ленина-33, Витебск Черняховского-4, Гродно Будённого-30). Stale-data risk: client noted "проспект Машерова, 6а" may be wrong — flagged for client verification, NOT silently deleted. Fix shipped: LLM system prompt extended with two clauses — (a) when asked "адреса офисов" plural without a city, list ALL cities from chunks; (b) NEVER mix "лизинг недвижимости" topic into office-address answers (different domains). KB content audit deferred to a verification pass with the client.
