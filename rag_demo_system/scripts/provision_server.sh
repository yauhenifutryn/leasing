...
STACK_QWEN_CMD="./.venv/bin/python -m vllm.entrypoints.openai.api_server --model Qwen/Qwen3-30B-A3B --port ${VLLM_PORT} --dtype bfloat16 --max-model-len 8192 --gpu-memory-utilization 0.85 --download-dir ${MODELS_DIR}"
...
#   Combines the main FAQ KB with the scraped website KB into one file.
#   The backend indexes a single markdown file (kb_markdown_path).
# ---------------------------------------------------------------------------
merge_knowledge_base() {
  ...
  if [ -f "$website_kb" ]; then
    ...
    # Append website content to the main KB file (backend reads kb_faq_ru.md)
    # Only append if not already merged (idempotent check)
...
main() {
  ...
  merge_knowledge_base       # Step 8b: combine FAQ + website KB into one file
