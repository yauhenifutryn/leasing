        ...
        try:
            classify_resp = await asyncio.to_thread(
                ...
                system_prompt=(
                    ...
                    # Bug 22 (Batch 4): explicit end-of-call signal.
                    "intent=END_CALL: клиент явно прощается / завершает разговор. Маркеры:\n"
                    "  'до свидания' / 'пока' / 'всего доброго' / 'спасибо, всё' / 'на этом всё' / 'больше ничего не нужно'.\n"
                    "  ВАЖНО: это intent=END_CALL ТОЛЬКО если клиент НЕ оставил незавершённое действие (нет change_field, нет вопроса). "
                    ...
                    "    'окей, понятно' -> intent=CONVERSATION (НЕ прощание, просто подтверждение)\n\n"
