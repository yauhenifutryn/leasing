...
def _fallback_response(intent: str) -> str:
    ...
    if intent == "identity":
        return "Я Ксения, голосовая помощница компании «Микро Лизинг». Чем могу помочь?"
    if intent == "meta":
        return "Я консультирую по услугам компании «Микро Лизинг». Чем могу вам помочь?"
    if intent == "off_topic":
        return (
...
def _llm_router_response(intent: str, message: str, base_url: str, model: str) -> str:
    system_prompt = (
        "Ты Ксения, голосовая помощница компании «Микро Лизинг». Ты женщина, используй женские формы глаголов (рада, готова, смогла). "
