...
class SIPAudioAdapter:
    ...
    async def read_next(self) -> dict[str, Any] | None:
        ...
        if frame_type == FRAME_UUID:
            # Asterisk 20 sends 36-char ASCII UUID from dialplan FILTER.
            # Asterisk 18 sent 16 raw bytes (binary UUID).
            if len(payload) == 16:
    ...
    async def write_audio(self, pcm16_24k: bytes) -> None:
        ...
        # Batch pacing: send 25 frames (500ms of audio), then drain + sleep.
        # This prevents Asterisk TCP buffer overflow while keeping playback smooth.
        # Tested and confirmed working at normal speech speed.
        batch_size = 25
        frame_count = 0
